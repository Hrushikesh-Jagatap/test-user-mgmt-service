...
const app = express();
const  pushNotifications  = require('@services/v1/Notification')
...
const sendOtp = async (userid,app_name) => {
  try {
    ...
    otpStores[userId] = { otp, expirationTime };
    ...
    appName: app_name,
      ...
      body: messages,
      title: 'You have received otp'
    }

    const Notificationservice = await pushNotifications.pushNotification(NotificationData);
...
const verifyOtp = (userId,otps) => {
  try {
    ...
    if (otpStores.hasOwnProperty(userId)) {
      const { otp, expirationTime } = otpStores[userId];
